...
export function Timeline({ chat }: { chat: ChatState }) {
  return <div className="space-y-8">{timelineGroups(chat).map((group) => {
    ...
    const progress = group.messages.filter((message) => message.id !== group.finalId);
    const final = group.messages.find((message) => message.id === group.finalId);
    const updates = <div className="space-y-4 border-l border-line pl-4 text-muted">{progress.map((message) => <Message key={message.id} message={message} progress showCopy={false} />)}</div>;
    return <section key={group.id} className="space-y-5" aria-label="Assistant turn">
      ...
      {final && <Message message={final} showCopy={group.settled} />}
    </section>;
  })}</div>;
}
