...
Today, Shepherd ships Discord and opt-in private web surfaces, plus a separate localhost signal webhook.
...
- `ui`: React + TypeScript + Tailwind interface, built with Vite
- `server/adapters/web`: built-in UI, private HTTP conversation API and SSE events
...
and failure behavior. The web surface serves the built-in UI and API from the
same listener. See
[the web UI guide](.docs/web-ui.md) for setup, development and recovery.
